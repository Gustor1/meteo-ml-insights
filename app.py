...
def build_llm_context(results: dict) -> str:
    """Construit le seul contexte agrégé transmis au LLM local."""
    ...
    walk_forward_summary = results.get("walk_forward_summary")
    ...
    if walk_forward_summary is not None and not walk_forward_summary.empty:
        walk_forward_text = "\n".join(
            f"- {row['modele']} : MAE moyenne {row['mae_moyenne']:.3f} °C ; "
            f"écart-type de MAE {row['mae_ecart_type']:.3f} °C ; "
            f"RMSE moyen {row['rmse_moyen']:.3f} °C ; "
            f"biais moyen {row['biais_moyen']:+.3f} °C ; "
            f"{int(row['nombre_fenetres'])} fenêtre(s)."
            for _, row in walk_forward_summary.iterrows()
        )
    else:
        walk_forward_text = "Validation temporelle glissante non exécutée."
...
- Distingue le test final unique de la validation temporelle glissante.
...
def show_results(results: dict) -> None:
    ...
    if walk_forward_summary is not None and not walk_forward_summary.empty:
        st.caption("Chaque fenêtre teste environ une année future. Le modèle est entraîné uniquement sur les données antérieures à cette période.")
        st.dataframe(walk_forward_summary, use_container_width=True, hide_index=True)
        ...
        st.info("La validation temporelle glissante n'a pas été exécutée.")
    ...
    st.markdown("### Graphiques de prédiction")
    ...
    for column, (title, image_path) in zip(graph_columns, graphs):
        if Path(image_path).exists():
            ...
            column.warning(f"Graphique introuvable : {image_path}")
    ...
    for column, (title, image_path) in zip(climate_columns, climate_graphs):
        if Path(image_path).exists():
            column.image(image_path, caption=title, use_container_width=True)
        else:
            column.warning(f"Graphique introuvable : {image_path}")
...
def main() -> None:
    ...
    if st.button("Lancer l'analyse complète", type="primary", disabled=uploaded_file is None):
        ...
        with st.spinner("Analyse locale : qualité, anomalies, modèles, validation temporelle et graphiques..."):
            try:
    ...
    question = st.chat_input("Exemple : Quel modèle est le plus stable selon les fenêtres temporelles ?")
    if question:
